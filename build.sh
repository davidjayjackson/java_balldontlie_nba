#!/usr/bin/env bash
#
# build.sh - Assemble the balldontlie NBA Java Calc add-in into build/NBA.oxt
#
# Pipeline:
#   1. unoidl-write : idl/**.idl           -> build/types/XNba.rdb   (UNO type library)
...
echo '[1/5] unoidl-write : idl -> build/types/XNba.rdb'
rdb="$type_dir/XNba.rdb"
"$uw" "$types" "$root/idl" "$rdb"

# --- 2. Generate Java bindings from the type library ------------------------
echo '[2/5] javamaker    : build/types/XNba.rdb -> build/gen'
...
echo '[4/5] jar          : classes + bindings -> build/oxt/nba.jar'
nba_jar="$stage/nba.jar"
mf="$root/registration/MANIFEST.MF"
# Merge classes + bindings into one tree first: this JDK's `jar` tool rejects
# duplicate directory entries (e.g. com/example/nba/) when given as two
...
"$jar" cfm "$nba_jar" "$mf" -C "$jar_dir" .

# --- 5. Stage remaining package files and zip the .oxt ----------------------
echo '[5/5] zip          : staging -> build/NBA.oxt'
cp "$rdb" "$stage/types/XNba.rdb"
...
echo
echo "Built $oxt"
echo 'Install with:  unopkg add --force build/NBA.oxt   (see docs/INSTALL.md)'
